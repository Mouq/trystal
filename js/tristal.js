...
(function(){
  var width, height, filldist, mindist, renderer, stage, tristalbox, x$, mass, bullseye, tempPoint1, tempPoint2, square, fallId, animate;
  width = 1000;
  height = 600;
  ...
  window.Tristal = {
    ...
    dropFalling: function(){
      var mass, rad, pre, k, ref$, obj, pos, vel, dir, stuck, stuckAt, edges, centers, i$, len$, ei, j$, len1$, ci, ref1$, ref2$, changed, xDist, yDist, theta, sc, cc, tc, rot_num, findex, fdir, results$ = [];
      mass = this.mass;
      rad = this.rad;
      pre = this.precision;
      for (k in ref$ = this.falling) {
        ...
        if (stuck) {
          ...
          fdir = ['nw', 'ne', 'se', 'sw'][(findex + rot_num) % 4];
          if (rot_num % 2 !== 0) {
            tempPoint2.x = pos.x;
            tempPoint2.y = pos.y;
            pos.x = tempPoint2.y;
          ...
          if (fdir === 'nw' && (tempPoint1.y < -2 * rad || tempPoint1.x < -2 * rad)) {
            console.log(1, k);
            pos.x = Math.min(yDist / sc - pos.y / tc, xDist / cc + pos.y * tc);
          } else if (fdir === 'se' && (tempPoint1.y > height + 2 * rad || tempPoint1.x > width + 2 * rad)) {
            console.log(2, k);
            pos.x = -Math.min(yDist / sc + pos.y / tc, xDist / cc - pos.y * tc);
          } else if (fdir === 'ne' && (tempPoint1.y < -2 * rad || tempPoint1.x > width + 2 * rad)) {
            console.log(3, k);
            pos.y = Math.min(yDist / cc - pos.x * tc, xDist / sc + pos.x / tc);
          } else if (fdir === 'sw' && (tempPoint1.y > height + 2 * rad || tempPoint1.x < -2 * rad)) {
            console.log(3, k);
            pos.y = -Math.min(yDist / cc + pos.x * tc, xDist / sc - pos.x / tc);
          }
          if (rot_num % 2 !== 0) {
